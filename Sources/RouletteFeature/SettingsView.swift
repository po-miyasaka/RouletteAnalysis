...
public struct Settings: ReducerProtocol {
    ...
    public func reduce(into state: inout State, action: Action) -> ComposableArchitecture.EffectTask<Action> {
        ...
        case .setup:
            ...
            return .none
        case .changeDefaultDisplayedHistoryLimit(let value):
...
struct SettingsView: View {
    ...
    struct ViewReducer: ReducerProtocol {
        ...
        func reduce(into state: inout State, action: Action) -> ComposableArchitecture.EffectTask<Action> {
            switch action {
            case .setActiveSheet(let activeSheet):
    ...
    init(store: Store<Settings.State, Settings.Action>) {
        self.settingsViewStore = ViewStore(store)
        ...
        WithCloseButton(dismissAction: { dismiss() }) {
            form()
                .sheet(
                    ...
                    case .request:
                        RequestView(store: .init(initialState: .init(), reducer: RequestFeature())).padding()
                    }
                }
        }
    ...
    @ViewBuilder
    func requestSection() -> some View {
        Section {
#if os(macOS)
            Button("Request / Feedback", action: {
                viewStore.send(.setActiveSheet(.request))
            })
            
#else
            NavigationLink<Text, RequestView>("Request / Feedback", destination: {
                RequestView(store: .init(initialState: .init(), reducer: RequestFeature()))
            })
#endif
